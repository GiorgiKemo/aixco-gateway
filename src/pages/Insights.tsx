    ...
    <>
      ...
      <main className="pt-32 pb-20">
        <section className="container-x">
          ...
          <div className="mt-12 flex flex-col md:flex-row md:items-center md:justify-between gap-6 border-y border-border/60 py-5">
            <div className="flex flex-wrap gap-2">
              {TAGS.map((t) => (
                <button key={t} onClick={() => setTag(t)} className={`text-[11px] uppercase tracking-widest px-3 py-1.5 rounded-sm border transition ${tag === t ? "border-primary text-primary bg-primary/5" : "border-border/60 text-muted-foreground hover:text-foreground"}`}>{t}</button>
              ))}
            </div>
            <label className="flex items-center gap-2 border border-border/60 rounded-sm px-3 py-2 md:w-72">
          ...
          <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((a) => (
              <Link key={a.slug} to={`/insights/${a.slug}`} className="group rounded-sm border border-border/60 bg-background p-7 hover:border-primary/60 transition flex flex-col">
                <div className={`mb-6 aspect-[4/3] rounded-sm bg-gradient-to-br ${a.hero} relative overflow-hidden`}>
                  <span className="absolute top-3 left-3 text-[10px] uppercase tracking-widest bg-background/80 px-2 py-1 rounded-sm">{a.tag}</span>
