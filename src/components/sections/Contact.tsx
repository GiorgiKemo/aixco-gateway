    ...
    <section id="contact" className="relative py-28 md:py-36 scroll-mt-24 bg-surface/40">
      <div className="container-x grid lg:grid-cols-12 gap-12">
        <div className="lg:col-span-5">
          ...
          <div className="mt-10 space-y-5">
            <a href={`mailto:${company.email}`} className="flex items-start gap-4 group">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-sm border border-border/60 group-hover:border-primary/60 transition"><Mail className="h-4 w-4 text-primary" /></span>
              <div>
            ...
            <div className="flex items-start gap-4">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-sm border border-border/60"><MapPin className="h-4 w-4 text-primary" /></span>
              <div>
        ...
        <div className="lg:col-span-7">
          {state === "success" ? (
            <div className="glass rounded-sm p-10 text-center animate-scale-in">
            ...
            <form onSubmit={onSubmit} noValidate className="grid gap-6 glass rounded-sm p-7 md:p-10">
              ...
              <div>
                ...
                <select id="interest" name="interest" defaultValue="bond" className="mt-1.5 w-full bg-transparent border-b border-border focus:border-primary outline-none py-2.5 text-sm">
              ...
              <div>
                ...
                <textarea id="message" name="message" rows={5} className="mt-1.5 w-full bg-transparent border-b border-border focus:border-primary outline-none py-2.5 text-sm resize-none" />
...
function Field({ label, name, type = "text", error }: { label: string; name: string; type?: string; error?: string }) {
  return (
    <div>
      ...
      <input id={name} name={name} type={type} required aria-invalid={!!error} className="mt-1.5 w-full bg-transparent border-b border-border focus:border-primary outline-none py-2.5 text-sm" />
