      ...
      <div className="container-x">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {dubaiFunds.map((f, idx) => (
            <article key={f.id} className="group relative overflow-hidden rounded-sm border border-border/60 bg-background hover:border-primary/40 transition-all">
              <div className="relative aspect-[16/10] overflow-hidden">
                ...
                <span className={`absolute top-4 left-4 rounded-sm px-2.5 py-1 text-[10px] uppercase tracking-widest ${f.status === "Live" ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>
                  {f.status}
                </span>
                <span className="absolute top-4 right-4 font-display text-5xl text-primary/70">0{idx + 1}</span>
              </div>
