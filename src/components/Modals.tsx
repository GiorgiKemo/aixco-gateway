...
export function Modals() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 animate-fade-in">
      <div className="absolute inset-0 bg-background/85 backdrop-blur-md" onClick={close} aria-hidden />
      <div role="dialog" aria-modal="true" className="relative w-full max-w-2xl max-h-[88vh] overflow-y-auto glass rounded-sm shadow-elegant animate-scale-in">
        <button aria-label="Close" onClick={close} className="absolute right-3 top-3 z-10 inline-flex h-9 w-9 items-center justify-center rounded-sm hover:bg-muted">
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          {[
            ...
            <a key={r.role} href={r.url} target="_blank" rel="noreferrer"
              className="group rounded-sm border border-border/70 p-4 hover:border-primary/60 transition">
      ...
      <ol className="space-y-4">
        {data.steps.map((s, i) => (
          <li key={i} className="flex gap-4">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-sm border border-primary/40 text-primary font-display text-lg">{i + 1}</span>
...
function TeamDetail({ data }: { data: { name: string; role: string; bio: string; image: string } }) {
  return (
    <div className="grid sm:grid-cols-[180px_1fr] gap-6">
      <img src={`/src/assets/${data.image}.jpg`} alt={data.name} className="w-full aspect-[4/5] object-cover rounded-sm grayscale" loading="lazy" />
      <div>
